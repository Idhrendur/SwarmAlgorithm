	...
	for (int particle = 0; particle < max_particles; ++particle)
	{
		...
		for (int dim = 0; dim < num_weights; ++dim)
		{
			particles[particle].velocity[dim] +=
				 (rho_1 * (particles[particle].best_position[dim] - particles[particle].position[dim])) +
				 (rho_2 * (global_best_position[dim] - particles[particle].position[dim]));
...
void network_swarm::test_particles()
{
	for (int particle = 0; particle < max_particles; ++particle)
	{
		...
		for (int set = 0; set < num_train_sets; ++set)
		{
			...
			{
				double difference;
				difference = 1.0 * results[output] - 1.0 * training_data[set].outputs[output];
